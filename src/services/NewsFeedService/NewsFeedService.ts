...
export class NewsFeed {
	...
	private routeAceessName = {
		...
		getPostsURL: (page: number, list: number): string => `${this.routeAceessName.BaseURL}/?page=${page}&list=${list}/`,
	...
	getPosts(page: number, list: number) {
		return this.httpClient.get<Post[]>(`${this.routeAceessName.getPostsURL(page, list)}`);
	}
	 ...
	 * @returns
	 */
	createPost(title: string, message: string, files?: FileInfoDto[], recipients?: RecipientsPost) {
	 ...
	 * @returns
	 */
	updatePost(id: string, title: string, message: string, files: FileInfoDto[], recipients: RecipientsPost) {
		return (
	...
	deletePost(id: string) {
		return this.httpClient.delete<Post>(`${this.routeAceessName.deletePostURL(id)}`);
	}
}
